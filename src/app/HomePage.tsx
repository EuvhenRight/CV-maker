...
import { HOME_FAQ_NL } from "@/lib/seo";

const HOME_FAQ_EN = [
  {
    question: "Is MaakMijnCV really free?",
    answer:
      "Yes. MaakMijnCV is fully free — no account, no paywall, no ads. You download your CV as a PDF without paying anything.",
  },
  {
    question: "Are the templates ATS-friendly?",
    answer:
      "Yes. All 14 templates use semantic HTML that Applicant Tracking Systems parse correctly. Four classic templates omit the photo — extra safe for tech recruiters and government roles.",
  },
  {
    question: "Where is my CV stored?",
    answer:
      "Only in your own browser (localStorage). Nothing is sent to a server. After 60 minutes of inactivity your data is wiped automatically — safe on shared machines.",
  },
  {
    question: "Can I download my CV as a PDF?",
    answer:
      "Yes. Every template exports directly to an A4 PDF via your browser, pixel-accurate to the preview. You can also 'Print' and 'Save as PDF' from the print dialog.",
  },
  {
    question: "Do I have to put a photo on my CV?",
    answer:
      "No. In 2026 many Dutch employers — especially in tech, at municipalities, and international companies — omit the photo to reduce bias. MaakMijnCV therefore ships 4 classic templates without a photo, plus 10 industry templates with one.",
  },
  {
    question: "Can I create a cover letter too?",
    answer:
      "Yes. MaakMijnCV includes a built-in cover-letter editor with per-profession sample letters. Download CV and letter as separate PDFs or together.",
  },
];
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0EFEA] text-[#1A1919]">
      ...
      <section id="faq" className="border-b border-[#e8e6df] bg-white">
        <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
          ...
          <p className="mt-2 text-sm text-[#555]">
            {locale === "en"
              ? "The short answers, before you open the editor."
              : "De korte antwoorden, voordat je de editor opent."}
          </p>
          <dl className="mt-6 divide-y divide-[#e8e6df]">
            {(locale === "en" ? HOME_FAQ_EN : HOME_FAQ_NL).map((item) => (
              <div key={item.question} className="py-4">
                <dt className="font-display text-sm font-semibold text-[#1A1919]">
                  {item.question}
                </dt>
                <dd className="mt-1.5 text-sm leading-relaxed text-[#555]">
                  {item.answer}
                </dd>
              </div>
            ))}
