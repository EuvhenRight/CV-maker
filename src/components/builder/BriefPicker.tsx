...
import { PROFESSIONS, type ProfessionId } from "@/lib/profession-templates";
import { buildProfessionBrief } from "@/lib/brief-templates";
...
export function BriefPicker({ trigger }: BriefPickerProps) {
  const { locale, t } = useLocale();
  const loadCoverLetter = useCVStore((s) => s.loadCoverLetter);
  const hasExistingLetter = useCVStore((s) =>
    hasMeaningfulLetter(s.cv.coverLetter),
  );
  const [open, setOpen] = React.useState(false);

  function onPick(id: ProfessionId) {
    if (hasExistingLetter && !confirm(t("brief.dialog.confirmReplace"))) return;
    loadCoverLetter(buildProfessionBrief(id, locale));
    setOpen(false);
  }
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[90vh] w-[min(960px,calc(100vw-24px))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-[#e8e6df] bg-white shadow-2xl">
...
function hasMeaningfulLetter(
  letter: ReturnType<typeof useCVStore.getState>["cv"]["coverLetter"],
): boolean {
